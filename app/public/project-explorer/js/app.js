...
let register = function () { 
    ...
    .then(data => {
        console.log(data);
        let inputProgram=document.getElementById('inputprogram');
        for (let i = 0; i < data.length; i++) {
            let programList = document.createElement('option');
            programList.innerHTML = data[i];
            inputProgram.appendChild(programList);
        }
    })
        ...
        .then(response => {
            if (response.status === "ok") {
                ...
                errors.forEach(error => {
                    ...
                    error_Alert.innerHTML += `<strong>${error}</strong><br>` //the div showing one error per line so we use <br>
                })
...
let index = async function(){
    ...
    if (window.location.href.includes('index.html')){
        ...
        //GET projects. All of them. Although, we are gonna be working with the first 4
        fetch('/api/projects/', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
